...
Server.prototype.handleClientEvents = function () {
    ...
    this.app.io.on('connection', function (socket) {
        ...
        socket.on('change_name', function (name, playerId) {
            console.log('Server/handleClientEvents | User wants to change the name');
            that.eventBus.emit('change_name', {
                name: name,
                id: playerId
            });
        });
...
Server.prototype.handleGameEvents = function () {
    ...
    this.eventBus.on('s_startNewGame', function (data) {
        console.log('Server/handleGameEvents | Game instance started a new game (host: %s).', data.host);
        that.emitNewGame(data);
    });
    
    this.eventBus.on('s_stopGame', function () {
        console.log('Server/handleGameEvents | Stopping game.');
        that.emitStopGame();
    });

    this.eventBus.on('s_newMessage', function (data) {
        that.app.io.emit('new_message', data);
    });
    
    this.eventBus.on('s_gameState', function (state) {
        that.app.io.emit('game_state', state);
    });
};
...
Server.prototype.emitNewGame = function (data) {
    var that = this;

    console.log('Sending request for start the game to all clients', data.host);
    that.app.io.emit('newGame', data);
};

Server.prototype.emitStopGame = function () {
    var that = this;

    console.log('Sending request for stop the game to all clients');
    that.app.io.emit('stopGame', {});
};
