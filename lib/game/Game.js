...
Game.prototype.initialize = function () {
    ...
    this.eventBus.on('new_answer', function (data) {
        ...
        if (that.isCorrectAnswer(data.answer)) {
            var timeElapsed = Game.TIME_FOR_GUESS - that.timer.getCurrentTime();
            var pointsForGuess = Game.TIME_FOR_GUESS - timeElapsed;
            that.timer.kill();
            
            that.onlinePlayers[that.inProgress].increasePoints(Game.POINTS_FOR_DRAWING);
            that.onlinePlayers['/#' + data.playerId].increasePoints(pointsForGuess);
            ...
            that.eventBus.emit('s_newMessage', {
                player: {name: Game.MASTER_NAME},
                message: {answer: that.onlinePlayers['/#' + data.playerId].getName() + ' sent correct answer! (+' + pointsForGuess + ' points)'}
            });
...
Game.PENALTY_POINTS = 20;
Game.POINTS_FOR_DRAWING = 50;
